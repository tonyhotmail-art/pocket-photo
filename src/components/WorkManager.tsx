...
// ─── 回收區面板（獨立組件，按需展開載入）────────────────────
function RecycleBin({ onRestored, tenantId }: { onRestored: () => void, tenantId: string }) {
    ...
    const load = useCallback(async () => {
        ...
        try {
            // 使用真正的 tenantId 進行查詢
            const res = await fetch(`/api/recycle?tenantId=${tenantId}`);
    ...
    const handleRestore = async () => {
        ...
        setWorking(true);
        try {
            await fetch(`/api/recycle?tenantId=${tenantId}`, {
                method: "PATCH",
    ...
    const handlePermanentDelete = async () => {
        ...
        try {
            ...
            await fetch(`/api/recycle?ids=${idList}&tenantId=${tenantId}`, { method: "DELETE" });
            setSelectedIds(new Set());
            await load();
        } catch (e) {
            alert("永久刪除失敗，請稍後再試");
...
export default function WorkManager({ tenantId }: { tenantId: string }) {
    ...
    useEffect(() => {
        ...
        loadPage(1);
        loadCategoryCounts();
    }, [selectedCategory, pageSize, tenantId]);
    ...
    const loadPage = async (page: number) => {
        ...
        setLoading(true);
        try {
            // 使用真正的 tenantId 進行分頁查詢
            const response = await fetch(
                `/api/works/paginate?page=${page}&pageSize=${pageSize}&category=${selectedCategory}&tenantId=${tenantId}`
            );
    ...
    const handleBatchPermanentDelete = async () => {
        ...
        try {
            const idList = Array.from(selectedIds).join(",");
            const res = await fetch(`/api/recycle?ids=${idList}&tenantId=${tenantId}`, { method: "DELETE" });
    ...
    return (
        <div className="w-full space-y-6 pb-20">
            ...
            <RecycleBin onRestored={() => { loadPage(1); loadCategoryCounts(); }} tenantId={tenantId} />
